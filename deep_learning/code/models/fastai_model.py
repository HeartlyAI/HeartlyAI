...
class fastai_model(ClassificationModel):
        ...
        if(self.pretrainedfolder is None): #from scratch
            ...
            if callbacks is not None:
                for callback in callbacks:
                    learn.callback_fns.append(callback(learn))
            ...
            # print(f"The loss function is {self.loss}")
    ...
    def predict(self, X):
        ...
        # print(self.name)
        learn.load(self.name, strict=False)
    ...
    def _get_learner(self, X_train,y_train,X_val,y_val,num_classes=None):
        ...
        # print(f"DS train shape: {ds_train.shape}")
        ...
        # print(f"In channels: {self.input_channels}, shape: {self.input_shape}")
        metrics = []
    ...
    def to_onnx(self, X, onnx_file_path):
        X = [l.astype(np.float32) for l in X]
        y_dummy = [np.ones(self.num_classes,dtype=np.float32) for _ in range(len(X))]
        ...
        dummy_shape = (1, self.input_channels, 250) # (batch_size, num_leads, sequence_length)
        ...
        torch.onnx.export(learn.model,               # model being run
                        ...
                        verbose=False,              # show the output ONNX model
                        ...
                        dynamic_axes={'input': {0: 'batch_size', 2: 'sequence_length'},  # variable-length axes
                                        'output': {0: 'batch_size'}})

        # torch.onnx.dynamo_export(learn.model, dummy_input)
