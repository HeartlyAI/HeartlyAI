...
wandb.login()
wandb.init(project='eit', entity='jorgensandhaug')

def temp_callback(learn):
    print(learn)
...
class WandbCallback():
    ...
    def _call_and_update(self, cb, cb_name, **kwargs):
        # print("Call and update called")
        return self
        
        
    def on_epoch_end(self, epoch, smooth_loss, last_metrics, **kwargs):
        "Log metrics and losses after each epoch."
        print(last_metrics)
        if wandb.run is None:
            wandb.init(project='eit', entity='jorgensandhaug')
        wandb.log({"loss": smooth_loss, "valid_loss": last_metrics[0]})
        return self
        
    def on_train_end(self, **kwargs):
        # print("Train end called")
        wandb.finish()
        return self

    def on_train_begin(self, **kwargs):
        # print("Train begin called")
        return self

    def on_epoch_begin(self, **kwargs):
        # print("Epoch begin called")
        return self

    def on_batch_begin(self, **kwargs):
        # print("Batch begin called")
        return self

    def on_batch_end(self, **kwargs):
        # print("Batch end called")
        return self

    def on_backward_begin(self, **kwargs):
        # print("Backward begin called")
        return self

    def on_backward_end(self, **kwargs):
        # print("Backward end called")
        return self
    
    def on_step_end(self, **kwargs):
        # print("Step end called")
        return self
    
    def on_step_begin(self, **kwargs):
        # print("Step begin called")
        return self 
    
    def on_loss_begin(self, **kwargs):
        # print("Loss begin called")
        return self
    
    def on_loss_end(self, **kwargs):
        # print("Loss end called")
        return self
...
class SCP_Experiment():
    ...
    def perform(self):
        ...
        for model_description in self.models:
            ...
            input_channels = modelparams['input_channels'] if 'input_channels' in modelparams else self.input_shape[1]
            print(f"Input channels: {input_channels} for model {modelname}")
            # Extract the first input_channels leads from each sample in data
            if isinstance(input_channels, str):
                i_from = int(input_channels.split('_')[0])
                i_to = int(input_channels.split('_')[1])
                X_train = np.array([d[:,i_from:i_to] for d in self.X_train])
                X_test = np.array([d[:,i_from:i_to] for d in self.X_test])
                X_val = np.array([d[:,i_from:i_to] for d in self.X_val])
             
                input_shape = (self.input_shape[0], i_to-i_from)
            else:
                X_train = np.array([d[:,:input_channels] for d in self.X_train])
                X_test = np.array([d[:,:input_channels] for d in self.X_test])
                X_val = np.array([d[:,:input_channels] for d in self.X_val])

                input_shape = (self.input_shape[0], input_channels)
            ...
            if modeltype == 'WAVELET':
                ...
                model = WaveletModel(modelname, n_classes, self.sampling_frequency, mpath, input_shape, **modelparams)
            elif modeltype == "fastai_model":
                from models.fastai_model import fastai_model
                model = fastai_model(modelname, n_classes, self.sampling_frequency, mpath, input_shape, **modelparams)
            elif modeltype == "YOUR_MODEL_TYPE":
                # YOUR MODEL GOES HERE!
                from models.your_model import YourModel
                model = YourModel(modelname, n_classes, self.sampling_frequency, mpath, input_shape, **modelparams)
            else:
                assert(True)
                break
            ...
            model.fit(X_train, self.y_train, X_val, self.y_val, callbacks=[WandbCallback])
            # # predict and dump
            # print(f'Predicting with model {modelname} on y_train')
            # model.predict(self.X_train).dump(mpath+'y_train_pred.npy')
            # print(f'Predicting with model {modelname} on y_val')
            # model.predict(self.X_val).dump(mpath+'y_val_pred.npy')
            # print(f'Predicting with model {modelname} on y_test')
