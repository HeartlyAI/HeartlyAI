...
def compute_label_aggregations(df, folder, ctype):
    ...
    if ctype in ['diagnostic', 'subdiagnostic', 'superdiagnostic', 'normabnorm']:

        def aggregate_all_diagnostic(y_dic):
            tmp = []
            for key in y_dic.keys():
                if key in diag_agg_df.index and y_dic[key] > 0:
                    tmp.append(key)
            return list(set(tmp))
        ...
        def aggregate_diagnostic(y_dic):
            tmp = []
            for key in y_dic.keys():
                if key in diag_agg_df.index and y_dic[key] > 0.75:
                    c = diag_agg_df.loc[key].diagnostic_class
                    if str(c) != 'nan':
                        tmp.append(c)
            return list(set(tmp))

        def normabnorm_aggregate_diagnostic(y_dic):
            for key in y_dic.keys():
                if key in diag_agg_df.index and y_dic[key] > 0.75:
                    c = diag_agg_df.loc[key].diagnostic_class
                    if c == "NORM":
        ...
        elif ctype == 'normabnorm':
            df['normdiagnostic'] = df.scp_codes.apply(normabnorm_aggregate_diagnostic)
...
def select_data(XX,YY, ctype, min_samples, outputfolder):
    ...
    if ctype == 'diagnostic':
        ...
        X = XX
        Y = YY
        mlb.fit(Y.normdiagnostic.values)
        y = mlb.transform(Y.normdiagnostic.values)
